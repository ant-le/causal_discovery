...
- **Acceptance Criteria:**
  - Run outputs live under `experiments/runs/{name}/` by default (Hydra `hydra.run.dir` and `hydra.sweep.dir`).
  - Output location can be overridden via `CAUSAL_META_RUN_DIR` / `CAUSAL_META_SWEEP_DIR`.
  - Each run contains `checkpoints/`, `results/`, and `inference/<model_name>/` subfolders.
